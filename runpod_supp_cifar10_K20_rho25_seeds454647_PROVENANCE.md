# PROVENANCE: Supplementary seeds 45/46/47 - CIFAR-10 K=20 rho=25

## Purpose
Raise Table 1 statistical power from n=3 (seeds 42/43/44) to n=6 (+ seeds
45/46/47) for the CIFAR-10 held-out weak config. Current paired t-test
...
## Reference
- Config source: `/mnt/e/fedprotrack/runpod_heldout_cifar10_all.json`
  (seeds 42, 43, 44 complete; 26 methods x 3 seeds in summary).
- Frozen config: `/mnt/e/fedprotrack/runpod_supp_cifar10_K20_rho25_seeds454647_config.json`

## Git state
- Git SHA at preparation: `55f7900` (verify with `git rev-parse HEAD` before submit;
  update this file if HEAD differs).
- Proposed tag: `exp/supp-cifar10-seeds454647-20260418`
- Create tag before submission:
  `cd /mnt/e/fedprotrack && git tag exp/supp-cifar10-seeds454647-20260418`
...
Notes:
- `--dataset cifar10` MUST be passed explicitly; the script's `--dataset`
  default is `cifar100`.
- CIFAR-10 internally uses `samples_per_class` (10 classes native),
  populated from the CLI's `--samples-per-coarse-class 120` value. The
  CIFAR-10 generator selects a subset of the 10 classes per concept.
...
- `--n-classes` is NOT a CLI flag for cifar10; class count is fixed by
  the dataset generator. The user-prompt template mentioned "--n-classes
  4 or 10"; the underlying benchmark script has no such argument.

## Expected output
- File: `/mnt/e/fedprotrack/runpod_supp_cifar10_K20_rho25_seeds454647.json`
...
- `dataset` -> "cifar10" (inferred from filename `runpod_heldout_cifar10_all.json`;
...
1. `jq 'keys' runpod_supp_cifar10_K20_rho25_seeds454647.json` -> ["45","46","47"]
