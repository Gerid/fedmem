...
On CIFAR-100 disjoint labels ($K{=}20$, $T{=}30$, 3~seeds), sweeping $C \in \{2,4,6,8\}$ via the recurrence period, FPT matches Oracle within 1pp at every $C$ value (Appendix~\ref{app:concept_count}).
...
\begin{table}[h]
\centering
\caption{Concept-count sensitivity (mean final accuracy over 3~seeds).
FPT matches Oracle at every $C$ and widens the gap to FedAvg as $C$ grows.}
\label{tab:csweep}
\small
\begin{tabular}{lcccc}
\toprule
...
Importantly, FPT's accuracy is insensitive to this over-estimation because empirical-Bayes shrinkage absorbs redundant concept slots (excess slots receive $\hat\lambda \to 1$, collapsing toward the global model).
...
\begin{table}[h]
\centering
\caption{Effect of misspecified cluster count $\hat C$ (true $C{=}4$).
...
FPT's accuracy is a constant $0.699$ across all $\hat C$ columns because it never uses $\hat C$---its OT spectral heuristic discovers $C$ from data.
Among cluster-aware baselines, IFCA and FeSEM degrade monotonically as $\hat C$ moves away from the true value.
...
FLUX-prior is the most robust cluster-aware baseline but still trails FPT by 3--4pp.
These results confirm that FPT's automatic concept discovery removes a critical hyperparameter that practitioners would otherwise need to tune via cross-validation or domain knowledge.
