...
"""Debug: adapter with frozen trunk (shared_update_policy)."""
...
configs = [
    ("linear ep=5", "linear", 0.05, 5, "always"),
    ("adapter ep=20 shared=always", "feature_adapter", 0.05, 20, "always"),
    ("adapter ep=20 shared=never", "feature_adapter", 0.05, 20, "never"),
    ("adapter ep=5 shared=never", "feature_adapter", 0.05, 5, "never"),
    ("adapter ep=20 shared=never", "feature_adapter", 0.05, 20, "never"),
    ("adapter ep=30 shared=never", "feature_adapter", 0.05, 30, "never"),
]

print(f"{'Config':40s}  {'FinalAcc':>8s}  {'MeanAcc':>8s}  {'Re-ID':>8s}  {'Bytes':>8s}")
print("-" * 75)

for name, mtype, lr, n_epochs, shared_policy in configs:
    ...
    result = FedProTrackRunner(
        ...
        shared_update_policy=shared_policy,
        **_common_kwargs(),
    ).run(dataset)
    ...
    print(f"{name:40s}  {result.final_accuracy:8.4f}  {result.mean_accuracy:8.4f}  "
