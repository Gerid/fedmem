...
"""Full 5-seed validation of adapter fix with recommended settings.

Compares:
- FPT-linear-base (ep=5, fed=2) -- original
- FPT-adapter-base (ep=20, fed=5) -- recommended adapter config
- FPT-adapter-base (ep=30, fed=5) -- more epochs variant
- CFL, IFCA baselines
"""
...
METHODS = [
    "FPT-linear-base",
    "FPT-adapter-ep20-fed5",
    "FPT-adapter-ep30-fed5",
    "CFL",
    "IFCA",
]
...
def run_single(method, dataset):
    ...
    if method == "FPT-linear-base":
        return FedProTrackRunner(
            ...
            lr=FPT_LR, n_epochs=5, soft_aggregation=True, blend_alpha=0.0,
            model_type="linear", similarity_calibration=False,
            model_signature_weight=0.0, model_signature_dim=8,
            prototype_alignment_mix=0.0, **common,
        ).run(dataset)

    if method == "FPT-adapter-ep20-fed5":
        return FedProTrackRunner(
            config=cfg, federation_every=5, detector_name="ADWIN", seed=seed,
            lr=FPT_LR, n_epochs=20, soft_aggregation=True, blend_alpha=0.0,
            model_type="feature_adapter", hidden_dim=64, adapter_dim=16,
            similarity_calibration=False, model_signature_weight=0.0,
            model_signature_dim=8, prototype_alignment_mix=0.0, **common,
        ).run(dataset)

    if method == "FPT-adapter-ep30-fed5":
        return FedProTrackRunner(
            config=cfg, federation_every=5, detector_name="ADWIN", seed=seed,
            lr=FPT_LR, n_epochs=30, soft_aggregation=True, blend_alpha=0.0,
            model_type="feature_adapter", hidden_dim=64, adapter_dim=16,
            similarity_calibration=False, model_signature_weight=0.0,
            model_signature_dim=8, prototype_alignment_mix=0.0, **common,
        ).run(dataset)
    ...
    if method == "IFCA":
        return run_ifca_full(dataset, federation_every=2, n_clusters=4)

    raise ValueError(f"Unknown method: {method}")


def main():
    results_dir = Path("tmp/cifar100_adapter_fixed")
    ...
    print("\n=== Adapter Fix Validation (mean +/- std over 5 seeds) ===", flush=True)
    print(
        f"{'Method':30s}  {'FinalAcc':>16s}  {'Re-ID':>16s}  {'Bytes':>8s}",
        flush=True,
    )
    print("-" * 75, flush=True)
    for method in METHODS:
        ...
        reid_str = (
            f"{np.mean(reids):.4f}+/-{np.std(reids):.4f}" if reids else "N/A"
        )
        bytes_str = f"{np.mean(bytes_vals):.0f}"
        print(f"{method:30s}  {acc_str:>16s}  {reid_str:>16s}  {bytes_str:>8s}", flush=True)
