...
"""Debug: adapter with different federation frequencies and epochs."""
...
SEED = 42
K = 4
T = 20  # use T=20 like the real experiment
N_SAMPLES = 400
N_FEATURES = 64
...
configs = [
    ("linear ep=5 fed=2", "linear", 0.05, 5, 2),
    ("adapter ep=5 fed=2", "feature_adapter", 0.05, 5, 2),
    ("adapter ep=20 fed=2", "feature_adapter", 0.05, 20, 2),
    ("adapter ep=20 fed=5", "feature_adapter", 0.05, 20, 5),
    ("adapter ep=30 fed=2", "feature_adapter", 0.05, 30, 2),
    ("adapter ep=30 fed=5", "feature_adapter", 0.05, 30, 5),
]

print(f"{'Config':35s}  {'FinalAcc':>8s}  {'MeanAcc':>8s}  {'Re-ID':>8s}  {'Bytes':>8s}")
print("-" * 70)

for name, mtype, lr, n_epochs, fed_every in configs:
    cfg = _build_cfg(dataset)
    result = FedProTrackRunner(
        config=cfg, federation_every=fed_every,
