...
"""H2h: Adapter learning rate sweep for concept signal preservation.

Tests whether reducing adapter lr slows loss convergence enough to
preserve the discriminative signal for Gibbs posterior concept detection.
...
def run_single(method_name: str, lr: float | None, dataset) -> object:
    seed = int(dataset.config.seed)
    common = _common_fpt_kwargs()
    cfg = _baseline_config(dataset)

    if method_name.startswith("adapter"):
...
def main() -> None:
    results_dir = Path("tmp/cifar100_h2h_lr")
    ...
    for seed in SEEDS:
        ...
        for method_name, lr in METHODS:
            done += 1
            print(f"[{done}/{total_combos}] seed={seed} {method_name} (lr={lr})...", flush=True)
            t0 = time.time()
            try:
                result = run_single(method_name, lr, dataset)
                log = _make_log(method_name, result, dataset.concept_matrix)
                canon = canonical_method_name(
                    "FedProTrack" if method_name != "CFL" else "CFL"
                )
    ...
    # Print summary table
    print("\n=== H2h LR Sweep Summary (mean +/- std over seeds) ===", flush=True)
    print(
        f"{'Method':20s}  {'FinalAcc':>14s}  {'Re-ID':>14s}  {'Entropy':>14s}  {'AUC':>14s}  {'Bytes':>8s}",
        flush=True,
    )
    print("-" * 95, flush=True)
    for method_name, _ in METHODS:
        subset = [r for r in all_rows if r["method"] == method_name]
        if not subset:
            continue
        ...
        print(
            f"{method_name:20s}  {acc_str:>14s}  {reid_str:>14s}  {ent_str:>14s}  {auc_str:>14s}  {bytes_str:>8s}",
            flush=True,
        )
